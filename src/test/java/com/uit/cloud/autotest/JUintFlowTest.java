...
public class JUintFlowTest {
	...
	@Variable("路径是字符串类型，长度是18到1024位，只允许数字、小写字母、中划线、以及圆点")
	public final static String PATH_PATTERN = "/var/lib/libvirt(/[a-z0-9-.]{2,32})*";
	
	@Variable("虚拟化支持的OS版本列表，详情执行osinfo-query os命令查看")
	...
	@Variable("虚拟交换机类型只能是bridge，或l2bridge，或l3bridge")
	public final static String SWITCH_TYPE_PATTERN = "bridge|l2bridge|l3bridge";
	
	@Variable("存储池类型，只能是dir, fs, netfs, disk, iscsi, logical, scsi, mpath, rbd, sheepdog, gluster, zfs, vstorage, iscsi-direct之一")
	...
	@Variable("IP范围，如192.168.1.0")
	public final static String IP_PATTERN  = "((25[0-5]|2[0-4]\\d|1\\d{2}|[1-9]?\\d)\\.){3}(25[0-5]|2[0-4]\\d|1\\d{2}|[1-9]?\\d)";
	
	@Variable("子网及其掩码，如192.168.1.0/24，掩码是8,16,24之一")
	public final static String SUBNET_PATTERN  = IP_PATTERN + "/(8|16|24)";
	
	@Variable("配置虚拟交换机，source是必填，ip和switch是选填")
	public final static String IP_SWITCH_PATTERN      = "source=(virbr0|br-native|br-int)(,ip=" + IP_PATTERN + ")?(,switch=([a-z0-9-.]{8,32}))?";
	
	@Variable("VCPU高级配置，允许绑定具体物理CPU，但cpuset是选填的")
	public final static String VCPUSET_PATTERN      = VCPU_PATTERN + "(,cpuset=\\d{1,3}((,|-)\\d{1,3})*)?";
	
	@Variable("配置基于vnc或者spice协议的虚拟机远程访问，密码是可选择的，为4-16位密码，是小写字母、数字和中划线组合")
	...
	public final static String VMI_PREFIX = "vmi.";
	
	public final static String VMD_PREFIX = "vmd.";
	
	public final static String VMDI_PREFIX = "vmdi.";
	...
	static {
		...
		computeTests.add(VirtualMachineDisk.class.getSimpleName() + "=" + CloneDisk.class.getName());
		computeTests.add(VirtualMachineDisk.class.getSimpleName() + "=" + DeleteDisk.class.getName());
		
		computeTests.add(VirtualMachine.class.getSimpleName() + "=" + StopVM.class.getName());
		computeTests.add(VirtualMachine.class.getSimpleName() + "=" + CloneVM.class.getName());
	...
	public static void main(String[] args) {
		System.out.println(NAME_TooLong.length());
	}
}
