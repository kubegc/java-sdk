...
public class AnnotationUtils {
    ...
    public final static String DESC_FUNCTION_DESC =
        "只会返回True或者异常" + "返回True意味着提交到Kubernetes成功，并不代表执行成功(异步设计)。开发人员需要通过监听Event和Watcher方法获取更详细信息；"
    ...
    public final static String DESC_TARGET_DESC =
        "windows可适用hdx驱动，Linux可适用vdx驱动，x是指a,b,c,d可增长，但不能重名，disk具体是哪种target，以及适用了哪些target可以通过get方法获取进行分析";

    public final static String DESC_FUNCTION_EXEC =
        "RuntimeException: 出现重名或不存在，或者该资源（如VirtualMachine, VirtualMachinePool等）还在处理中，需等待上一步操作完成才能进行下一步";

    public final static String DESC_FUNCTION_VM =
    ...
    public final static String DESC_FUNCTION_OPENSTACK_SERVER = "Server存在，即已调用过CreateServer";
